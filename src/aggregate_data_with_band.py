...
def load_data_and_activities(given_day_path):
    ...
    all_files = (
        glob.glob(os.path.join(given_day_path, "*", "*.csv")) +
        glob.glob(os.path.join(given_day_path, "BAND_user1", "*.csv"))
    )
    if not all_files:
    ...
    for f in all_files:
        ...
        if parent_dir == "BAND_user1" and file_name == "gyro.csv":
            continue
        ...
        if "activity_user" in file_name:
            ...
            if parent_dir == "BAND_user1":
                # Load all columns except the first one (timestamp)
                cols_to_load.extend(all_file_cols[1:].tolist())
            else:
        ...
        try:
            ...
            valid_cols_to_load = list(pd.unique(valid_cols_to_load))
...
def densify_dataset(sparse_input_df):
    ...
    # Ensure no duplicates enter the densification process
    sparse_input_df = sparse_input_df.loc[:, ~sparse_input_df.columns.duplicated()]
...
def rename_columns(df):
    ...
    for col in df.columns:
        ...
        # Vibration_v1_state -> v1 (for v1-v11)
            ...
            new_name = col.split('_')[1]
        elif col.startswith('BAND_user1_'):
            # Splits 'BAND_user1_filename_columnname' into [BAND_user1, filename, columnname]
            parts = col.split('_', 2)
            if len(parts) == 3:
                # The new name is the original column name, which is 'calories', 'bpm', etc.
                new_name = parts[2]
...
def normalize_data(df):
    ...
    # These columns are renamed to their column header (e.g., 'calories', 'bpm', 'gsr', etc.)
    band_cols = ['calories', 'bpm', 'fat_burned', 'meters', 'steps']
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(SAVE_PATH, "final_processed_data_ALL_DAYS_small_window_with_band.csv")
